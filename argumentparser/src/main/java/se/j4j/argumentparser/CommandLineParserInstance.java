...
import static se.j4j.argumentparser.Argument.ParameterArity.NO_ARGUMENTS;
...
import static se.j4j.strings.StringsUtil.startsWithAndHasMore;
...
final class CommandLineParserInstance
{
			...
			throws ArgumentException
	{
		...
		if(isCommandParser())
		{
			...
			// TODO: this also means that Commands will have been executed if the command line was
			// invalid, how to prevent this?
			arguments.previous();
			return null;
		}
	...
	private Set<String> availableArguments(final ParsedArguments holder)
	{
		Set<String> validArguments = Sets.newHashSetWithExpectedSize(allArguments().size());
		for(ArgumentSettings argument : allArguments())
		{
			if(!holder.hasParsed(argument) || argument.isAllowedToRepeat())
			{
